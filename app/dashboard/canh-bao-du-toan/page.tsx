...
export default function CanhBaoDuToanPage() {
  ...
  const { budgetAlerts } = useMemo(() => getBudgetMonitoringSnapshot(), []);

  const visibleAlerts = useMemo(
    () => filterBudgetAlerts(budgetAlerts, { period: filterPeriod, risk: filterRisk }),
    [budgetAlerts, filterPeriod, filterRisk],
  );

  const chartData = visibleAlerts.map((item) => ({
    name: item.TenKhoanMuc.length > 15 ? `${item.TenKhoanMuc.substring(0, 15)}...` : item.TenKhoanMuc,
    'Dự toán': item.DuToan / 1000000,
    'Đã chi': item.DaChi / 1000000,
  }));

  const nghiemTrong = visibleAlerts.filter((c) => c.MucDo === 'Nghiêm trọng').length;
  const trungBinh = visibleAlerts.filter((c) => c.MucDo === 'Trung bình').length;
  const theoDoi = visibleAlerts.filter((c) => c.MucDo === 'Theo dõi').length;
  const anToan = visibleAlerts.filter((c) => c.MucDo === 'An toàn').length;
            ...
            <Dialog>
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Tóm tắt mức độ rủi ro và nhóm khoản mục cần ưu tiên xử lý.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 md:grid-cols-2 py-2">
                  <div className="rounded-lg border p-4 bg-slate-50/70">
                    ...
                    <div className="space-y-1 text-sm">
                      <p>
                        Nghiêm trọng: <strong className="text-red-600">{nghiemTrong}</strong>
                      </p>
                      <p>
                        Trung bình: <strong className="text-amber-600">{trungBinh}</strong>
                      </p>
                      <p>
                        Theo dõi: <strong className="text-blue-600">{theoDoi}</strong>
                      </p>
                      <p>
                        An toàn: <strong className="text-green-600">{anToan}</strong>
                      </p>
                    </div>
                  </div>
                  <div className="rounded-lg border p-4 bg-slate-50/70">
                    ...
                    <p className="text-2xl font-semibold text-red-600">
                      {visibleAlerts.filter((c) => c.TyLeThucHien > 100).length}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      trên tổng số {visibleAlerts.length} khoản mục theo dõi
                    </p>
                  </div>
                </div>
                <DialogFooter>
            ...
            <BarChart data={chartData}>
              ...
              <Bar dataKey="Đã chi" fill="#ef4444">
                {chartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={visibleAlerts[index].TyLeThucHien > 100 ? '#ef4444' : '#10b981'}
                  />
                ))}
              </Bar>
            </BarChart>
              ...
              <div key={item.MaKhoanMuc} className="border rounded-lg p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="text-right">
                    <div
                      ...
                      style={{ color: item.TyLeThucHien > 100 ? '#ef4444' : '#10b981' }}
                    >
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>
                      Dự toán: <strong>{formatCurrency(item.DuToan)}</strong>
                    </span>
                    <span>
                      Đã chi: <strong>{formatCurrency(item.DaChi)}</strong>
                    </span>
                    <span className={item.DaChi > item.DuToan ? 'text-red-600 font-semibold' : 'text-green-600'}>
                      {item.DaChi > item.DuToan ? '+' : ''}
                      {formatCurrency(item.DaChi - item.DuToan)}
                    </span>
                  </div>
